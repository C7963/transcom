...
namespace PWDCONFIG
{
	...
	{
	public:
		static PWDCtrl* getInstance() {
			if (instance == nullptr)
			{
				instance = new PWDCtrl();
			}
			return instance;
		};
		static PWDCtrl* instance;
		PWDCtrl();
		~PWDCtrl();
		void Caculate_F_IFs(int64_t* Nfreq);
		void PWD_Enable(bool enable);
		void PWD_Threshold(uint32_t channel_num, float* threshold);
		void PWD_Threshold(uint32_t channel_num, uint32_t threshold);
		//void PWD_DDC_Config(uint32_t channel_num, uint32_t ftw, uint32_t phase);
		void PWD_DDC_Config();
		void PWD_DDC_Decim(uint32_t decim);
		void PWD_DDC_FIR_COEF(uint32_t* value, uint32_t length);
	...
	{
	public:
		static PWDStream* getInstance() {
			if (instance == nullptr)
			{
				instance = new PWDStream();
			}
			return instance;
		};
		PWDStream();
	...
	private:
		...
		PWDCtrl* pwdctrl;
		DeviceDataConfig::DeviceDataAPI* device;
